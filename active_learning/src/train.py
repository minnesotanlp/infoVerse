...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    logs = {}
                    if (
                        args.local_rank == -1 and args.evaluate_during_training_epoch
        ...
        if args.save_model_during_training:
            ...
            pre_ckpt = os.path.join(args.output_dir, f"seed{args.seed}_epoch{epoch}")
            if not os.path.exists(pre_ckpt):
                os.makedirs(pre_ckpt)
            model_to_save.save_pretrained(pre_ckpt)
...
def save_model(args, model, tokenizer, epoch, best_epoch,  best_dev_performance):
    results = evaluate(args, model, tokenizer, prefix="in_training")
...
def evaluate(args, model, tokenizer, prefix="", data_split='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, evaluate=True, data_split=data_split)
        ...
        if args.output_mode in ["classification", "multiple_choice"]:
        ...
        output_eval_file = os.path.join(eval_prefix_dir, f"eval_results_{prefix}_{args.seed}.txt")
        with open(output_eval_file, "w") as writer:
            logger.info("***** Eval results {} *****".format(data_split))
            for key in sorted(result.keys()):
                logger.info("  %s = %f", key, result[key])
                logger.info(f"{eval_task} {data_split} {key} = {result[key]:.4f}")
                writer.write("%s = %f\n" % (key, result[key]))
...
def main():
    ...
    args.evaluate_during_training_epoch = False
    ...
    if args.do_eval or args.do_test and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            for eval_split in eval_splits:
                result = evaluate(args, model, tokenizer, prefix=eval_split, data_split=eval_split)
